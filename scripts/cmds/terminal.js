...
function kv(icon, key, val) { return `${icon} ${key}: ${val}`; }
...
async function _run({ args, ctx, event }) {
    ...
    if (sub === "help" || sub === "?") {
        return reply(box("🖥️ TERMINAL v7", [
            "🔐 ACCOUNT & COOKIE",
            ...
            `  list      — list account files`,
            `  token     — current session info`,
    ...
    if (sub === "config" || sub === "cfg") {
        ...
        if (!key) {
            const lines = Object.entries(cfg)
                ...
                .map(([k, v]) => kv("▸", k.slice(0, 20), String(typeof v === "object" ? JSON.stringify(v).slice(0, 40) : v).slice(0, 40)));
    ...
    if (sub === "url" || sub === "cookies" || sub === "dashboard") {
